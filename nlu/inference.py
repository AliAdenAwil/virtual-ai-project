...
class JointNLUPredictor:
    ...
    def _apply_slot_fallbacks(
        ...
        if intent == "SetTimer" and "duration" not in p:
            m = re.search(
                r"\b(\d+)\s*(seconds?|minutes?|hours?)\b", t, re.IGNORECASE
            )
            if not m:
                m = re.search(
                    r"\b(one|two|three|four|five|ten|fifteen|twenty|thirty)\s*(seconds?|minutes?|hours?)\b",
                    t, re.IGNORECASE,
                )
            if m:
        ...
        # ── PlayMovie / AddToWatchlist ──
        if intent in {"PlayMovie", "AddToWatchlist"} and "title" not in p:
            for pat in [
                r"\b(?:play|watch|start)\s+(?:movie\s+)?(.+?)(?:\s+movie)?$",
                r"\b(?:add|save|put)\s+(.+?)\s+(?:to\s+watchlist|in\s+watchlist)",
            ]:
                m = re.search(pat, t, re.IGNORECASE)
                if m:
                    cand = cls._clean(m.group(1))
            ...
            if "song_title" not in p:
                for pat in [
                    ...
                    if m:
                        ...
                        if not re.search(r"\b(playlist|music|song|track|artist|album)\b", song, re.IGNORECASE):
                            p["song_title"] = song
                            break
            if "artist_name" not in p:
                m = re.search(r"\bby\s+([A-Za-z][A-Za-z\s]{1,30})\b", t, re.IGNORECASE)
