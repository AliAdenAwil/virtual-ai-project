...
def _truncate_at_sentence(text: str, max_chars: int = 500) -> str:
    """Truncate only at sentence boundaries (. ? !) near max_chars."""
    ...
    lookahead = text[max_chars:max_chars + 120]
    next_end = _SENTENCE_END_RE.search(lookahead)
    if next_end:
        return text[:max_chars + next_end.end()].strip()

    # No sentence boundary nearby: keep the original text rather than cut mid-sentence.
    return text
...
def _trim_fulfillment(fr: dict) -> dict:
    ...
    for k, v in fr.items():
        ...
        if isinstance(v, str) and len(v) > 500:
            trimmed[k] = _truncate_at_sentence(v, max_chars=500)
        else:
            trimmed[k] = v
